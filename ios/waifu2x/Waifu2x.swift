...
public class Waifu2x {
  ...
  static public func run(
    ...
    var out_scale: Int
    switch model {
    case .anime_noise0, .anime_noise1, .anime_noise2, .anime_noise3, .photo_noise0, .photo_noise1,
      .photo_noise2, .photo_noise3:
      ...
      alpha_task = BackgroundTask("alpha") {
        if out_scale > 1 {
          ...
          if let metalBicubic = try? MetalBicubic() {
            ...
            if out_width <= metalBicubic.maxTextureSize()
              && out_height <= metalBicubic.maxTextureSize()
            {
    ...
    // Output
    Waifu2x.out_pipeline = BackgroundPipeline<MLMultiArray>("out_pipeline", count: rects.count) {
      (index, array) in
    ...
    callback("processing")
    Waifu2x.in_pipeline = BackgroundPipeline<CGRect>(
      "in_pipeline", count: rects.count,
      task: { (index, rect) in
        let x = Int(rect.origin.x)
        let y = Int(rect.origin.y)
        let multi = try! MLMultiArray(
          shape: [
            3, NSNumber(value: Waifu2x.block_size + 2 * Waifu2x.shrink_size),
            NSNumber(value: Waifu2x.block_size + 2 * Waifu2x.shrink_size),
          ], dataType: .float32)
        var x_new: Int
        var y_new: Int
        for y_exp in y..<(y + Waifu2x.block_size + 2 * Waifu2x.shrink_size) {
          for x_exp in x..<(x + Waifu2x.block_size + 2 * Waifu2x.shrink_size) {
            x_new = x_exp - x
            y_new = y_exp - y
            multi[y_new * (Waifu2x.block_size + 2 * Waifu2x.shrink_size) + x_new] = NSNumber(
              value: expanded[y_exp * expwidth + x_exp])
            var dest =
              y_new * (Waifu2x.block_size + 2 * Waifu2x.shrink_size) + x_new
              + (block_size + 2 * Waifu2x.shrink_size) * (block_size + 2 * Waifu2x.shrink_size)
            multi[dest] = NSNumber(value: expanded[y_exp * expwidth + x_exp + expwidth * expheight])
            dest =
              y_new * (Waifu2x.block_size + 2 * Waifu2x.shrink_size) + x_new
              + (block_size + 2 * Waifu2x.shrink_size) * (block_size + 2 * Waifu2x.shrink_size) * 2
            multi[dest] = NSNumber(
              value: expanded[y_exp * expwidth + x_exp + expwidth * expheight * 2])
          }
        }
    ...
    let cgImage = CGImage(
      ...
      bytesPerRow: out_width * channels, space: colorSpace,
      bitmapInfo: CGBitmapInfo.init(rawValue: bitmapInfo), provider: dataProvider, decode: nil,
